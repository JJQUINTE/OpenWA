...
- API keys carry an optional `allowedChats` allowlist next to `allowedSessions`: a curated set of chats (groups and/or contacts) a key may reach, `null`/empty meaning unrestricted and independent of the session scope. A chat-restricted key is fenced **by default**: `ApiKeyGuard` answers `403` on every route not marked `@ChatScoped`, so webhooks, automation rules, status, key management, channels and every route added later stay closed without enumerating them, and such a key cannot manage API keys (a chat-scoped admin no longer counts toward the last-admin invariant). A marked route declares how it is safe — `'fenced'` (the `:chatId` / `:groupId` / `:contactId` path param, or a required `chatId` / `fromChatId` / `toChatId` / `messages[].chatId` body field), `'filtered'` (returns through `ChatScopeService.filter`), or `'agnostic'` (no chat reach, granted explicitly). An out-of-scope chat is `403`, a present-but-non-string chat field is `400`, and `quotedMessageId` is refused outright (a quote resolves from the global store, outside the allowlist). `send-bulk` verifies every recipient in `messages[].chatId`, while its batch status and cancel routes stay unmarked — a restricted key gets `403` from the returned `statusUrl`. Identity resolves through the `lid_mappings` table (read persistently, not from the evictable cache): a phone entry matches its resolved `@lid` form and vice versa, a lid's digits are never read as a phone, and an unmapped `@lid` stays outside the fence. `/events`, the MCP mount (per tool call) and the Bull Board queue dashboard refuse such a key. Of the list routes, only `GET /sessions/:sessionId/chats` is usable — and it filters before paginating. A structural coverage spec checks every mark against its declared category ([#1634](https://github.com/rmyndharis/OpenWA/issues/1634)).
