...
- Message forwarding (`POST .../messages/forward`) and bulk message sending (`POST .../messages/bulk`) enforce the API key `allowedChats` allowlist: forward verifies both `fromChatId` and `toChatId`, while bulk send verifies every recipient in `messages[].chatId`, rejecting any unauthorized chat with `403` ([#1634](https://github.com/rmyndharis/OpenWA/issues/1634)).
- API keys carry an optional `allowedChats` allowlist next to `allowedSessions`: a curated set of chats (groups and/or contacts) a key may reach, `null`/empty meaning unrestricted and independent of the session scope. A chat-restricted key is fenced **by default**: the `ApiKeyGuard` answers `403` on every route not marked `@ChatScoped`, so webhooks, automation rules, status, key management, channels and any route added later are closed without enumerating them, and the key cannot manage API keys (a chat-scoped admin no longer counts toward the last-admin invariant). On a marked route the guard checks every chat id it can see — the `:chatId` / `:groupId` / `:contactId` path param, `?chatId=`, and the `chatId` / `fromChatId` / `toChatId` / `messages[].chatId` body fields — rejecting an out-of-scope chat with `403` and a present-but-non-string chat field with `400`. Identity is resolved through the lid mapping table (`lid_mappings`, read deterministically rather than from the evictable cache): a phone entry matches its resolved `@lid` form and vice versa, a lid's digits are never read as a phone, and an unmapped `@lid` stays outside the fence. `GET /events` and the MCP mount refuse such a key at their handshake until the event/tool surfaces are filtered. A structural coverage spec fails if a marked handler is neither fenceable nor filters through `ChatScopeService`.
